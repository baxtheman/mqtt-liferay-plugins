...
<c:choose>
	<c:when test="<%= Validator.isNotNull(signature) %>">
		<div class="lfr-api-method lfr-api-section">
			...
			<div id="readMore">
				<button class="header btn toggler-header-collapsed">
			...
			Method proxyactionMethod = jsonWebServiceActionMapping.getActionMethod();

			Method actionMethod = actionClass.getMethod(proxyactionMethod.getName(),
				proxyactionMethod.getParameterTypes());
			%>
			...
			<div class="lfr-api-param">
				...
				if (actionClassName.contains(".service.") && actionClassName.endsWith("ServiceImpl")) {
		...
		<div class="lfr-api-execute lfr-api-section">
			...
			<div class="lfr-api-results" id="serviceResults">
			...
			<aui:form action="<%= jsonWSPath + invocationPath %>" enctype="<%= enctype %>" method="<%= jsonWebServiceActionMapping.getMethod() %>" name="execute">
				...
				%>
						...
						<aui:input id='<%= "field" + i %>' label="<%= methodParameterName %>"
							name="<%= methodParameterName %>" suffix="<%= methodParameterTypeClassName %>" type="file" />
					...
					<%
						...
						String paramValue = ParamUtil.getString(request, methodParameterName);
					%>
				...
				%>
					...
					<aui:button type="submit" value="Invoke service" />
				...
				>
					<liferay-ui:section>
						<button title="copy to clipboard" id="copy-clipboard" class="btn" data-clipboard-target="#serviceOutput">
							<i class="icon-copy"></i>
						</button>
